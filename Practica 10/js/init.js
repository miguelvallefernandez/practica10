    ...
    */
    ...
    function homeEvent(store) {
        return function () {
            ...
            if (document.getElementById("papelera")) {
            ...
            if (document.getElementById("ubicaciones")) {
                document.getElementById("ubicaciones").remove();
            }
            ...
            while (shop.done !== true) {
                if (shop.value.name == tiendaParam) {
                    ...
                    while (shops.done !== true) {
                        ...
                        div1.setAttribute("id", "drag" + i);
                        i++;


                        document.getElementById("papelera").addEventListener("dragstart", function (event) {
                            event.dataTransfer.setData("Text", event.target.id);
                        });

                        document.getElementById("papelera").addEventListener("dragover", function (event) {
                            event.preventDefault();
                        });

                        document.getElementById("papelera").addEventListener("drop", function (event, prdct, sp) {
                            event.preventDefault();
                            var data = event.dataTransfer.getData("Text");
                            event.target.appendChild(document.getElementById(data));
                            document.getElementById(data).remove();
                            eliminarProducto(store, prdct, sp);
                        });


                        // div1.addEventListener("dragend", eliminarProducto(store, shops.value.product, shop.value));
...
// Open the indexedDB.
    ...
    function agregarTienda(store) {
        return function () {
            ...
            var coordenadas = new Coords(latitud, longitud);

            tienda.coords = coordenadas;

            console.log(coordenadas);
            console.log(tienda.coords);
...
// Open the indexedDB.
    ...
    function rellenarForm(tienda) {
        return function () {
            ...
            document.getElementById("latitud1").setAttribute("value", tienda.coords.latitude);
            document.getElementById("longitud1").setAttribute("value", tienda.coords.longitude);
    ...
    function modificarTienda(tienda) {
        return function () {
            ...
            tienda.coords.latitude = document.getElementById("latitud1").value;
            tienda.coords.longitude = document.getElementById("longitud1").value;
            ...
            request.onsuccess = function (event) {
                ...
                request.onsuccess = function (event) {
                    ...
                    data.coords.latitude = tienda.coords.latitude;
                    data.coords.longitude = tienda.coords.longitude
    ...
    function abrirBaseDatos() {
        ...
        while (shop.done !== true) {
            ...
            shopData.push({
                ...
                coords: {latitude: shop.value.coords.latitude, longitude: shop.value.coords.longitude},
                productos: productos
            });
        ...
        request.onsuccess = function (event) {
            ...
            objectStore.openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    var coordenadas = new Coords(cursor.value.coords.latitude, cursor.value.coords.longitude);

                    shop.coords = coordenadas;
                ...
                if (cursor) {
                    ...
                    tiendas.push({
                        shop: {
                            ...
                            coords: {latitude: cursor.value.coords.latitude, longitude: cursor.value.coords.longitude},
                            productos: productos
                        }
                    });
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (var i in tiendas) {
                    ...
                    if (tiendas[i].shop.cif != "1111") {
                        ...
                        shop.coords = tiendas[i].shop.coords;
                    ...
                    if (cursor) {
                        ...
                        tiendas.push({
                            shop: {
                                ...
                                image: cursor.value.image,
                                coords: {
                                    latitude: cursor.value.coords.latitude,
                                    longitude: cursor.value.coords.longitude
                                },
                                productos: productos
                            }
                        });
...
function mapas() {
    ...
    var a = document.createElement("a");
    a.setAttribute("href", "#");
    a.setAttribute("id", "ubicaciones");
    divb.setAttribute("class", "ubicaciones pull-right")
    var texto = document.createTextNode("Ubicación");
    a.appendChild(texto);
    divb.appendChild(a)
    menu.appendChild(divb);

    document.getElementById("ubicaciones").addEventListener("click", dibujarMapa());
    ...
    function dibujarMapa() {
        return function () {
            ...
            document.getElementById("home").innerHTML = "Volver atrás";
...
function myMap() {
    ...
    while (shop.done !== true) {
        ...
        coordenadas.push(shop.value);

        /*
                var myCenter = new google.maps.LatLng(shop.value.coords.latitude, shop.value.coords.longitude);
    ...
    for (i = 0; i < coordenadas.length; i++) {
        marker = new google.maps.Marker({
            position: new google.maps.LatLng(coordenadas[i].coords.latitude, coordenadas[i].coords.longitude),
            map: map
        });
        google.maps.event.addListener(marker, 'click', (function (marker, i) {
            return function () {
                infowindow.setContent(coordenadas[i].name);
                infowindow.open(map, marker);
            }
        })(marker, i));
    }
    ...
    /*
        var map = new google.maps.Map(document.getElementById('mapa-agregar'), {
            zoom: 6,
            center: new google.maps.LatLng(40.46366700000001, -3.7492200000000366),
            mapTypeId: google.maps.MapTypeId.ROADMAP
        });
    ...
    var mapCanvas = document.getElementById("mapa-agregar");
    var myCenter = new google.maps.LatLng(40.46366700000001, -3.7492200000000366);
    var mapOptions = {center: myCenter, zoom: 5};
    var map = new google.maps.Map(mapCanvas, mapOptions);
    ...
    var mapCanvas2 = document.getElementById("mapa-editar");
    var myCenter2 = new google.maps.LatLng(40.46366700000001, -3.7492200000000366);
    ...
    function placeMarker2(map, location) {
        ...
        document.getElementById("latitud1").value = location.lat();
        document.getElementById("longitud1").value = location.lng();
...
function drop(ev) {
    ...
    //document.getElementById("drag2").remove();
    //console.log(store.removeProductInShop(prd, sp));
    document.getElementById(data).remove();
